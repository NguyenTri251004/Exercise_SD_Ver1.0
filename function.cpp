...
const string phonePattern = R"(^(\+84|0[3|5|7|8|9])\d{8}$)";
...
bool kiemTraSDTHopLe(const string& sdt) {
    return regex_match(sdt, regex(phonePattern));
}

bool kiemTraTinhTrang(const string& oldTinhTrang, const string& newTinhTrang) {
    if (oldTinhTrang == "Dang hoc") {
        return newTinhTrang == "Bao luu" || newTinhTrang == "Da tot nghiep" || newTinhTrang == "Dinh chi";
    }
    else if (oldTinhTrang == "Da tot nghiep") {
        return false;
    }
    return true;
}
...
void capNhatSinhVien() {
    ...
    while (true) {
        cout << "Nhap tinh trang moi: ";
        getline(cin, tinhtrang);
        if (kiemTraTinhTrang(studentList[MSSV].getTinhtrang(), tinhtrang)) {
            break;
        }
        cout << "Khong the doi tinh trang tu '" << studentList[MSSV].getTinhtrang() << "' sang '" << tinhtrang << "'!\n";
    }
